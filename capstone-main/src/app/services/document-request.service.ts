...
export class DocumentRequestService {
  ...
  private apiUrl = 'http://16.176.16.134:8000'; // <-- Replace with your FastAPI AWS URL
  ...
  async syncUnsyncedRequests(): Promise<void> {
    try {
      ...
      const payload = unsynced.map(r => ({
        ...
        clientId: r.id  // send SQLite ID to mark synced
      }));

      this.http.post(`${this.apiUrl}/sync-requests/`, payload).subscribe({
        next: async (res: any) => {
          if (res.synced_ids && Array.isArray(res.synced_ids)) {
            for (const id of res.synced_ids) {
              await this.markAsSynced(id); // mark SQLite as synced
            }
          }
          console.log('✅ Unsynced requests synced.');
        },
